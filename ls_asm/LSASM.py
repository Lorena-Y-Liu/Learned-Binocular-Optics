...
class LeastSamplingASM():
    def __init__(self, Uin, xvec, yvec, z, device):
        ...
        z = torch.as_tensor(z, device=device)#.repeat(3,1)
        ...
        #if 1 - (wavelength[0] * fxmax)**2 - (wavelength[0] * fymax) ** 2 <= 0:
        ...
        #if Uin.type == "12" or Uin.type == "123":
        ...
        deltax = torch.tensor(0)#self.compute_shift_of_H(FHcx, FHbx + Uin.D, xc, wx)
        deltay = torch.tensor(0)#self.compute_shift_of_H(FHcy, FHby + Uin.D, yc, wy)
        ...
        #tau_UHx = min(tau_UHx.min(), tau_fx_bound.min()) + 41.2 / Uin.fbX
        #tau_UHy = min(tau_UHy.min(), tau_fy_bound.min()) + 41.2 / Uin.fbY
        #dfxMax1 = 1 / tau_UHx
        #dfyMax1 = 1 / tau_UHy
        ...
        LRfx = 1520 #math.ceil(Lfx / dfx * Uin.s*0.8)
        LRfy = 1520 #math.ceil(Lfy / dfy * Uin.s*0.8)
    ...
    def __call__(self, E0):
        ...
        #F=abs(Fu[:,0,...])/ abs(Fu[:,0,...]).max() #.sum(dim=(-2, -1), keepdims=True)
        #FF=torch.fft.fft2(Fu[:,0,...]).abs()
        #FF=torch.fft.fftshift(FF)/ FF.max()
        #torchvision.utils.save_image(F.cpu(), 'psf.jpg')
        #torchvision.utils.save_image(FF.cpu(), 'Fpsf.jpg')
