"""
This is where the input field is integrated.

This code and data is released under the Creative Commons Attribution-NonCommercial 4.0 International license (CC BY-NC.) In a nutshell:
    # The license is only for non-commercial use (commercial licenses can be obtained from authors).
    # The material is provided as-is, with no warranties whatsoever.
    # If you publish any code, data, or scientific work based on this, please cite our work.


Technical Paper:
Haoyu Wei, Xin Liu, Xiang Hao, Edmund Y. Lam, and Yifan Peng, "Modeling off-axis diffraction with the least-sampling angular spectrum method," Optica 10, 959-962 (2023)
"""


import numpy as np
from ls_asm.phase_plates import SphericalWave, ThinLens
from operator import add 
from ls_asm.utils import effective_bandwidth
import torch
class InputField():
    '''
    Prepare compensated input field and spatial sampling
    '''
    def __init__(self, type, wvls, r, z0, f, zf, mask_size, modulate_phase, heightmap):# -> None:

        self.wvls = wvls  # wavelength of light in vacuum
        self.k = 2 * torch.pi / self.wvls  # wavenumebr
        #thetaX, thetaY = angles #0
        thetaX=torch.tensor(0)
        thetaY=torch.tensor(0)
        self.modulate_phase = modulate_phase
        self.heightmap = heightmap
        s=1
        # define incident wave
        r0 = z0 / torch.sqrt(1 - torch.sin(thetaX / 180 * torch.pi)**2 - torch.sin(thetaY / 180 * torch.pi)**2)
        x0, y0 = r0 * torch.sin(thetaX / 180 * torch.pi), r0 * torch.sin(thetaY / 180 * torch.pi)
       
        # prepare wave components
        typelist = [*type]
        wavelist = []
        fcX = 0  # frequency centers
        fcY = 0


        if "1" in typelist:
            
            field = SphericalWave(self.k, x0, y0, z0, (0,0), zf)
            fcX += field.fcX
            fcY += field.fcY
            wavelist.append(field)

        if "2" in typelist:
            lens = ThinLens(self.k, f)
            fcX += lens.fcX
            fcY += lens.fcY
            wavelist.append(lens)

        # Compute spatial sampling
        Nx, Ny, fbX, fbY = self.spatial_sampling(r, s, wavelist, mask_size)
        
        # Prepare input field
        self.set_input_plane(r, Nx, Ny)
        E0 = self.pupil
        for wave in wavelist:
            E0 = wave.forward(E0, self.xi_, self.eta_)

        self.fcX = fcX
        self.fcY = fcY
        self.fbX = fbX
        self.fbY = fbY
        self.E0 = E0
        self.s = s
        self.zf = zf
        self.D = 2 * r
        self.type = type
        self.mask_size = mask_size


    def spatial_sampling(self, r, s, wavelist, mask_size):
        '''
        :param r: aperture radius
        :param s: oversampling factor
        :param wavelist: a list of input wave components
        :return: number of samples in both dimensions, bandwidths in both dimensions
        '''

        # Second term in Eq4, the size of Airy disk
        #fplane = effective_bandwidth(r*2, is_plane_wave = True)

        # First term in Eq4, maximum phase gradient
        # as the phase terms are all monotonic here, 
        # we use the two boundaries of aperture (+-r) to find max
        #grad1 = torch.tensor([0, 0])
        #grad2 = torch.tensor([0, 0])
        #diffuser = False
        #for wave in wavelist:
            #if isinstance(wave, Diffuser):
                #diffuser = True
                #grad = wave.phase_gradient()
                #fbX_diffuser = grad[0] * s
                #fbY_diffuser = grad[1] * s
            #else:
                #grad1 = list(map(add, grad1, wave.phase_gradient(-r, -r))) 
                #grad2 = list(map(add, grad2, wave.phase_gradient(r, r))) 
        #fbX = (torch.max(abs(grad1[0]), abs(grad2[0])) / torch.pi + fplane) * s
        #fbY = (torch.max(abs(grad1[1]), abs(grad2[1])) / torch.pi + fplane) * s

        #if diffuser:
            #fbX = max(fbX.max(), fbX_diffuser)
            #fbY = max(fbY.max(), fbY_diffuser)

        Nx= torch.tensor(mask_size)#(torch.ceil_(fbX * 2 * r)).int().max()
        Ny= torch.tensor(mask_size)
        #Nx = (torch.ceil_(fbX * 2 * r)).int().max() #torch.tensor(1000)#(torch.ceil_(fbX * 2 * r)).int().max()
        #Ny = (torch.ceil_(fbY * 2 * r)).int().max() #torch.tensor(1000)#(torch.ceil_(fbY * 2 * r)).int().max()
        return Nx, Ny, (Nx - 1) / (2 * r), (Ny - 1) / (2 * r)


    def set_input_plane(self, r, Nx, Ny):

        # coordinates of aperture
        xi = np.linspace(-r, r, Nx, endpoint=True)
        eta = np.linspace(-r, r, Ny, endpoint=True)
        xi_, eta_ = np.meshgrid(xi, eta, indexing='xy')

        # circular aperture
        pupil = np.where(xi_**2 + eta_**2 <= r**2, 1, 0)

        self.pupil = pupil
        self.xi, self.eta = xi, eta
        self.xi_, self.eta_ = xi_, eta_
