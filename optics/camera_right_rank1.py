...
class MixedCamera(BaseCamera):
    def __init__(self, focal_depth: float, min_depth: float, max_depth: float, n_depths: int,
        ...
        #init_heightmap1d_x_0 = torch.ones((mask_size//2 // mask_upsample_factor)).cuda().float()*1e-6

        n_param=mask_size//2 // mask_upsample_factor
        n= torch.tensor(n_param).cuda()
        #init_heightmap1d_x_0 = (torch.arange((n_param),0,-1)*5/n_param).cuda().float()*1e-6
        
        x=torch.arange(0,n_param,1).cuda()
        init_heightmap1d_x_0 = 2*(1-(torch.square(x))/torch.square(n)).cuda().float()
        
        init_heightmap1d_y_0 = torch.ones(n_param).cuda().float()*2#2*torch.ones(n_param).cuda().float()

        '''init_heightmap1d_x_0=torch.load('x_0_r_rank1.pth')
        init_heightmap1d_y_0=torch.load('y_0_r_rank1.pth')'''
        ...
        self.heightmap1d_x_1 = torch.nn.Parameter(init_heightmap1d_x_0, requires_grad=requires_grad)
        self.heightmap1d_y_1 = torch.nn.Parameter(init_heightmap1d_y_0, requires_grad=requires_grad)
        

        self.h_max=0.55/0.5625*1e-6#1.1*1e-6#1.1*1e-6
    ...
    def heightmap1d_x(self):
        '''torch.save(self.heightmap1d_x_0.to(torch.device('cpu')), 'x_0_r_rank1.pth')
        torch.save(self.heightmap1d_y_0.to(torch.device('cpu')), 'y_0_r_rank1.pth')
        torch.save(self.heightmap1d_x_1.to(torch.device('cpu')), 'x_1_r_rank1.pth')
        torch.save(self.heightmap1d_y_1.to(torch.device('cpu')), 'y_1_r_rank1.pth')'''
    ...
    def heightmap1d_y(self):
        '''return F.interpolate(self.heightmap1d_y_0.reshape(1, 1, -1),
                             scale_factor=1, mode='nearest').reshape(-1)'''
        return F.interpolate(self.heightmap1d_y_0.reshape(1, 1, -1),
                             scale_factor=self.mask_upsample_factor, mode='nearest').reshape(-1)
    ...
    def aperture(self):
        ...
        return aperture
    def height(self):
        '''heightmap2d=self.heightmap2d()#torch.sigmoid(self.heightmap2d()-1)
        heightmap2d=copy_quadruple(heightmap2d.unsqueeze(0).unsqueeze(0))
        #height_map=heightmap2d.unsqueeze(0).unsqueeze(0)
        ...
        height_map=scio.loadmat('does/Rank1/Rank1_L.mat')['phase']
        ...
        height_map=height_map*self.aperture() #+(1-self.aperture())*0.5
    ...
    def phase(self):
        #h_max=0.85*1e-6
        heightmap=torch.remainder(self.height(),1)#self.height()
    ...
    def psf_out_of_fov_energy(self, psf_size: int):
        ...
        #[3,5,1200,1200]
        #Fp = fftshift(rfft(psf_diffracted, 2),dims=(-2, -3))
        #torchvision.utils.save_image(psf_diffracted[:,0,...]/psf_diffracted[:,0,...].max(), 'psf_diffracted.png')
        #torchvision.utils.save_image(Fp[:,0,...,0], 'Fpsf_diffracted.png')
