"""
Rank-1 DOE (Diffractive Optical Element) camera module for right camera.

This module implements a camera with Rank-1 low-rank DOE parameterization.
The Rank-1 structure represents the height map as a single outer product:
    h(x,y) = y_0 * x_0^T

The right camera uses a -45 degree rotation compared to the left camera's +45 degrees.

When use_pretrained_doe=True, the DOE height map is loaded from a .mat file.
When use_pretrained_doe=False, the DOE is parameterized by learnable 1D vectors.
"""
...
class MixedCamera(BaseCamera):
    """
    Right camera with Rank-1 parameterized diffractive optical element.

    The DOE height map is parameterized as:
        h(x,y) = y_0 * x_0^T

    The right camera uses a -45 degree rotation (opposite to left camera)
    to create complementary PSFs for stereo depth estimation.

    When use_pretrained_doe is True, the height map is loaded directly from
    a .mat file (does/Rank1/Rank1_R.mat) and is not optimizable.
    When False, learnable 1D vectors are used and the height map is constructed
    via outer product + copy_quadruple + rotation.
    """
    ...
    def _init_doe_parameters(self, requires_grad: bool):
        """Initialize the Rank-1 DOE parameters analytically for right camera."""
        n_param = self.mask_size // (2 * self.mask_upsample_factor)
        n = torch.tensor(n_param, dtype=torch.float32)
        x = torch.arange(n_param, dtype=torch.float32)

        # Right camera: swapped initialization compared to left
        init_x_0 = 2 * (1 - torch.square(x) / torch.square(n)).float()
        init_y_0 = 2 * torch.ones(n_param).float()
    ...
    def heightmap2d(self) -> torch.Tensor:
        """Compute 2D height map as a single outer product (Rank-1)."""
        return torch.matmul(
    ...
    def height(self) -> torch.Tensor:
        ...
        When use_pretrained_doe=True: loads from does/Rank1/Rank1_R.mat
        When use_pretrained_doe=False: uses learnable parameters
            1. Rank-1 outer product
            2. Copy to all four quadrants (symmetry)
            3. Rotate by -45 degrees (right camera)
            4. Wrap to [0, 1] range
            5. Mask by circular aperture
        """
        if self.use_pretrained_doe:
            import scipy.io as scio
            height_map = scio.loadmat('does/Rank1/Rank1_R.mat')['phase']
            ...
            heightmap2d = self._rotate_height(heightmap2d, -torch.pi / 4)  # -45 for right
