    ...
    def _capture_impl(self, volume, layered_depth, psf, occlusion, eps=1e-3):
        ...
        if occlusion:
            ...
            blurred_cumsum_alpha = irfft(
                complex.multiply(Fcumsum_alpha, Fpsf), 2, signal_sizes=volume.shape[-2:-1])
...
class MixedCamera(BaseCamera):
    def __init__(self, focal_depth: float, min_depth: float, max_depth: float, n_depths: int,
        ...
        """
        #Rank 2 Initialization

        init_heightmap_rank2_x_0 = torch.zeros(2,(mask_size // mask_upsample_factor)).float()*1e-6
        self.heightmap_rank2_x_0 = torch.nn.Parameter(init_heightmap1d_x_0, requires_grad=requires_grad)
        init_heightmap_rank2_y_0 = torch.ones(2,(mask_size // mask_upsample_factor)).float()*1e-6
        self.heightmap_rank2_y_0= torch.nn.Parameter(init_heightmap1d_y_0, requires_grad=requires_grad)
        """
        
        #Ring Coding Initialization

        init_heightmap1d_x_0 = torch.zeros((mask_size // 2// mask_upsample_factor)).cuda().float()#*1e-6

        #init_heightmap1d_x_0=torch.load('x_l_ring.pth')
        self.heightmap_x_0 = torch.nn.Parameter(init_heightmap1d_x_0, requires_grad=requires_grad)
    ...
    """
    # Rank 2 Coding
    def heightmap_rank2_x(self):
        return F.interpolate(self.heightmap1d_x_0.unsqueeze(0),
                             scale_factor=self.mask_upsample_factor, mode='linear')
    def heightmap_rank2_y(self):
        return F.interpolate(self.heightmap1d_y_0.unsqueeze(0),
                             scale_factor=self.mask_upsample_factor, mode='linear')
    """
    ...
    def heightmap2d(self):
        return torch.matmul(self.heightmap_rank2_y().squeeze(0).transpose(0,1),self.heightmap_rank2_x().squeeze(0))
    """
    def heightmap_x(self):
        #torch.save(self.heightmap_x_0.to(torch.device('cpu')), 'x_l_ring.pth')
        return F.interpolate(self.heightmap_x_0.reshape(1, 1, -1),
                             scale_factor=self.mask_upsample_factor, mode='nearest').reshape(-1)
    
    # Ring Coding
    def find_index(self, a, v):
        a = a.squeeze(1)
        index=torch.stack([torch.searchsorted(a[i, :], v[i]) - 1 for i in range(a.shape[0])], dim=0)
        return index
    def heightmap2d(self):
        heightmap1d = torch.cat([self.heightmap_x().cpu(), torch.zeros((self.mask_size // 2))], dim=0)
        heightmap1d = heightmap1d.reshape(1, 1, -1)
        r_grid = torch.arange(0, self.mask_size, dtype=torch.double)
        #theta_grid= torch.arange(0, 2*math.pi, self.mask_size, dtype=torch.double)
        ...
        #r_coord = torch.arctan(y_coord/x_coord).unsqueeze(0)
        ...
        heightmap = copy_quadruple(heightmap11).squeeze().cuda()
        #heightmap=cv2.imread('ring_left.png')[...,0]/255
        #heightmap=torch.from_numpy(heightmap).cuda()
        return heightmap
    ...
    def aperture(self):
        ...
        return aperture
    def height(self):
        '''#heightmap2d=torch.sigmoid(self.heightmap2d())#
        heightmap2d=self.heightmap2d()
        #heightmap2d=copy_quadruple(heightmap2d.unsqueeze(0).unsqueeze(0))
        height_map=heightmap2d
        #height_map=self.rot_height(heightmap2d, (torch.pi/4))
        height_map=height_map.squeeze(0).squeeze(0)'''

        import scipy.io as scio
        height_map=scio.loadmat('does/Ring/Ring_L.mat')['phase']
        height_map=(torch.from_numpy(height_map)/(2*torch.pi)).to(self.device)
        height_map=F.interpolate(height_map.unsqueeze(0).unsqueeze(0),
                             scale_factor=self.mask_upsample_factor, mode='nearest').squeeze(0).squeeze(0) 
    ...
    #def height(self):
        #heightmap2d=torch.sigmoid(self.heightmap2d())
        #heightmap2d=self.heightmap2d()*1e12+0.5
        #height_map=heightmap2d.squeeze()
        #return height_map
    def phase(self):
        h_max=0.55/0.5625*1e-6#1.1*1e-6
        heightmap=torch.remainder(self.height(),1)
        k = 2 * torch.pi / self.wavelengths.reshape(-1,1,1,1)
        phase = heightmap*k *h_max * (refractive_index(self.wavelengths.reshape(-1,1,1,1)) - 1)
    ...
    def psf_obs(self, Ein):
        #r = self.focal_length/self.f_number/2
        Mx, My = [256,256]#self.mask_size, self.mask_size
        l=5.86e-6*Mx
        '''r = self.focal_length/self.f_number/2
    ...
    def psf_obs_full(self, Ein):
        r = self.focal_length/self.f_number/2
        l = r * 0.25
        Mx, My = self.full_size, self.full_size #self.mask_size, self.mask_size
        xc=0; yc=0
        z=1 / (1 / self.focal_length - 1 / self.focal_depth)
        x = torch.linspace(-l / 2 + xc, l / 2 + xc, Mx)
        y = torch.linspace(-l / 2 + yc, l / 2 + yc, My)    
        device = self.device
        prop2 = LeastSamplingASM(self, x, y, z, device)
        U2 = prop2(Ein).cuda()
        return abs(U2)**2
    ...
    def psf_at_camera(self, size, modulate_phase, is_training=torch.tensor(False)):
        ...
        if is_training:
            ...
            #scene_distances = ips_to_metric(
                #torch.linspace(0, 1, steps=5, device=device) +
                #1 / 5 * (torch.rand(5, device=device) - 0.5),
                #self.min_depth, self.max_depth)
            ...
            scene_distances = ips_to_metric(torch.linspace(0, 1, steps=self.n_depths, device=device),
                                            self.min_depth, self.max_depth)
            #scene_distances = ips_to_metric(torch.linspace(0, 1, steps=5, device=device),
                                            #self.min_depth, self.max_depth)
        '''if modulate_phase: 
        ...
        if is_training:
            max_shift = 2
            r_shift = tuple(torch.random.randint(low=-max_shift, high=max_shift, size=2))
            b_shift = tuple(torch.random.randint(low=-max_shift, high=max_shift, size=2))
    ...
    def psf_out_of_fov_energy(self, psf_size: int):
        ...
        dist2=torch.where(dist>10, 1, 0).squeeze(0)

        #dist= torch.sqrt((X+0.5 - center) ** 2 + (Y+0.5 - center) ** 2).unsqueeze(0).unsqueeze(0).cuda()
        #grid = F.affine_grid(self.rot_matric(torch.pi/4).repeat(dist.shape[0],1,1), size=dist.shape).cuda()
        #dist2=F.grid_sample(dist, grid, mode='bilinear', padding_mode='border')
        #dist2=torch.where(dist>10, 1, 0).squeeze(0).squeeze(0)

        
        #mask_c[..., :, :]=torch.where(dist>10, 1, 0)
        mask_c[..., :, :]=dist2
        ...
        return psf_out_of_fov.sum()/10 #2000
