...
class MixedCamera(BaseCamera):
    def __init__(self, focal_depth: float, min_depth: float, max_depth: float, n_depths: int,
        ...
        init_heightmap1d_x_0=torch.load('doe_vector/x_0_r.pth')
        init_heightmap1d_y_0=torch.load('doe_vector/y_0_r.pth')
        init_heightmap1d_x_1=torch.load('doe_vector/x_1_r.pth')
        init_heightmap1d_y_1=torch.load('doe_vector/y_1_r.pth')
        ...
        self.h_max=0.55/0.5625*1e-6#1.1*1e-6
    ...
    def heightmap2d(self):
        #print(self.heightmap1d_x_0, self.heightmap1d_y_0, self.heightmap1d_x_1, self.heightmap1d_y_1)
        
        #print(torch.matmul(self.heightmap1d_y().reshape(-1,1),self.heightmap1d_x().reshape(1,-1)))
    ...
    def height(self):
        ...
        height_map=scio.loadmat('does/Rank2/rank2_r.mat')['phase']
        ...
        height_map=F.interpolate(height_map.unsqueeze(0).unsqueeze(0),
                             scale_factor=self.mask_upsample_factor, mode='nearest').squeeze(0).squeeze(0) #size=(self.n_depths, psf.shape[-2], psf.shape[-1])
        '''
        #print(height_map[...,630,559:561])
        height_map=torch.remainder(height_map, 1)
        height_map=height_map*self.aperture() #+(1-self.aperture())*0.5
        #torchvision.utils.save_image()
        #height_map=(height_map//(1/16))*(1/16)
