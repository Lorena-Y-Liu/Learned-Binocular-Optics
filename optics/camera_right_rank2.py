"""
Rank-2 DOE (Diffractive Optical Element) camera module for right camera.
...
class MixedCamera(BaseCamera):
    """
    Right camera with Rank-2 parameterized diffractive optical element.
    ...
    PSF_OBS_SIZE = 256  # Fixed: Keep consistent with left camera
    ...
    def _init_doe_parameters(self, requires_grad: bool, use_pretrained_doe: bool):
        ...
        if use_pretrained_doe:
            # Load pretrained DOE vectors for right camera
            init_x_0 = torch.load('doe_vector/x_0_r.pth')
            init_y_0 = torch.load('doe_vector/y_0_r.pth')
            init_x_1 = torch.load('doe_vector/x_1_r.pth')
            init_y_1 = torch.load('doe_vector/y_1_r.pth')
        else:
            # Initialize DOE vectors analytically for right camera
    ...
    def height(self) -> torch.Tensor:
        ...
        3. Rotated by -45 degrees (for right camera, opposite to left)
    ...
    def psf_at_camera(
        ...
        if not self.experiment:
            ...
            # Use captured PSF for experiments (index 1 for right camera)
    ...
    def psf_out_of_fov_energy(self, psf_size: int) -> torch.Tensor:
        ...
        outer_mask = torch.where(dist > 10, 1, 0)  # Fixed: Remove .squeeze(0), keep consistent with left camera
        mask[..., :, :] = outer_mask
