...
class MixedCamera(BaseCamera):
    def __init__(self, focal_depth: float, min_depth: float, max_depth: float, n_depths: int,
        ...
        init_heightmap1d_x_0 = torch.ones(n_param).cuda().float()*2#2*torch.ones(n_param).cuda().float()
        
        y=torch.arange(0,n_param,1).cuda()
        init_heightmap1d_y_0 = 2*(1-(torch.square(y))/torch.square(n)).cuda().float()
        
        '''init_heightmap1d_x_0=torch.load('x_0_l_rank1.pth')
        init_heightmap1d_y_0=torch.load('y_0_l_rank1.pth')'''
    ...
    def heightmap1d_x(self):
        '''torch.save(self.heightmap1d_x_0.to(torch.device('cpu')), 'x_0_l_rank1.pth')
        torch.save(self.heightmap1d_y_0.to(torch.device('cpu')), 'y_0_l_rank1.pth')'''
    ...
    def psf_out_of_fov_energy(self, psf_size: int):
        ...
        #torchvision.utils.save_image(dist2.unsqueeze(0).float(), 'dist2.png')
