"""
Rank-1 DOE (Diffractive Optical Element) camera module for left camera.
...
class MixedCamera(BaseCamera):
    """
    Left camera with Rank-1 parameterized diffractive optical element.

    The DOE height map is parameterized as:
        h(x,y) = y_0 * x_0^T

    When use_pretrained_doe is True, the height map is loaded directly from
    a .mat file (does/Rank1/Rank1_L.mat) and is not optimizable.
    ...
    def _init_doe_parameters(self, requires_grad: bool):
        """Initialize the Rank-1 DOE parameters analytically."""
