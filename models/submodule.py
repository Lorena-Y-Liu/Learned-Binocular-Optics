...
def disparity_regression_2d(x, maxdisp):
    assert len(x.shape) == 4
    disp_values = torch.arange(0, maxdisp, dtype=x.dtype, device=x.device)
    disp_values = disp_values.view(1, maxdisp, 1, 1)
    return torch.sum(x * disp_values, 1, keepdim=False)

def image_regression(x, maxrange):
    assert len(x.shape) == 5
    r=x[:,0,...]
    g=x[:,1,...]
    b=x[:,2,...]
    values = torch.arange(0, maxrange, dtype=r.dtype, device=r.device)
    values = values.view(1, maxrange, 1, 1)
    r_matrix=torch.sum(r * values, 1, keepdim=True)
    g_matrix=torch.sum(g * values, 1, keepdim=True)
    b_matrix=torch.sum(b * values, 1, keepdim=True)
    image=torch.cat([r_matrix,g_matrix,b_matrix], dim=1).squeeze(2)
    return image
