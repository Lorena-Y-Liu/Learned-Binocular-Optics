...
def mae(image1: torch.Tensor, image2: torch.Tensor) -> torch.Tensor:
    """Compute Mean Absolute Error between two tensors.
    
    Returns a scalar tensor on the same device as input (not a Python float),
    so that PyTorch Lightning can properly aggregate metrics at epoch end.
    """
    return F.l1_loss(image1, image2)


def mse(image1: torch.Tensor, image2: torch.Tensor) -> torch.Tensor:
    """Compute Mean Squared Error between two tensors.
    
    Returns a scalar tensor on the same device as input.
    """
    return F.mse_loss(image1, image2)


def rmse(image1: torch.Tensor, image2: torch.Tensor) -> torch.Tensor:
    """Compute Root Mean Squared Error between two tensors.
    
    Returns a scalar tensor on the same device as input.
    """
    return torch.sqrt(F.mse_loss(image1, image2))


def calculate_psnr(image1: torch.Tensor, image2: torch.Tensor) -> torch.Tensor:
    """
...
def calculate_ssim(img1: torch.Tensor, img2: torch.Tensor) -> torch.Tensor:
    ...
    Returns:
        SSIM value as scalar tensor on the same device as input.
    """
...
def calculate_3px(depth1: torch.Tensor, depth2: torch.Tensor, threshold: float = 3.0) -> torch.Tensor:
    """
    ...
    Returns:
        Percentage of pixels with error > threshold as scalar tensor.
    """
    abs_diff = torch.abs(depth1 - depth2)
    error_mask = abs_diff > threshold
    return error_mask.float().mean() * 100