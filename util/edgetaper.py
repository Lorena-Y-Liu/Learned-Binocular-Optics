...
try:
    ...
    def irfft4(x, d, signal_sizes):
        t = torch.fft.ifft2(torch.complex(x[:,:,:,:,0], x[:,:,:,:,1]), dim = (-d,-1))
        return t.real
    def irfft5(x, d, signal_sizes):
        t = torch.fft.ifft2(torch.complex(x[:,:,:,:,:,0], x[:,:,:,:,:,1]), dim = (-d,-1))
        return t.real
...
def edgetaper3d(img, psf):
    ...
    assert (psf.dim() == 5)
    psf = psf.mean(dim=-3)
    alpha = compute_weighting_for_tapering(psf)*1e-4
    blurred_img = irfft4(
        complex.multiply(rfft(img, 2), rfft(psf, 2)), 2, signal_sizes=img.shape[-2:]
    )
